  ...
  class SectionHeaderComponent < ViewComponent::Base
    ...
    def call
      content_tag(:div, class: "ui-section-header") do
        safe_join([
          title_block,
          actions? ? content_tag(:div, actions, class: "ui-section-header__actions") : nil
        ].compact)
      end
    end
    ...
    def title_block
      content_tag(:div, class: "ui-section-header__titles") do
        title_html = safe_join([
          @icon.present? ? content_tag(:i, "", class: "#{@icon} ui-section-header__icon") : nil,
          @title
        ].compact, " ")

        safe_join([
          content_tag("h#{@level}", title_html, class: "ui-section-header__title"),
          @subtitle.present? ? content_tag(:p, @subtitle, class: "ui-section-header__subtitle") : nil
        ].compact)
      end
    end
  end
