...
# Normalized weather alert for the homepage bar (LibreWXR and/or OpenWeather).
class WeatherAlert
  attr_reader :event, :description, :starts_at, :ends_at, :source

  def initialize(event:, description: nil, starts_at: nil, ends_at: nil, source: nil)
    @event = event
  ...
  def self.for_homepage(lat:, lon:, forecast_alerts: [])
